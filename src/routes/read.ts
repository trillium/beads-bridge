...
// Durable object format for the persistent prompt: read per-hit from
// blurbs/action-object.md (frontmatter stripped) so edits apply live.
function durableObjectFormat(): string {
  try {
    return stripFrontmatter(readFileSync(path.join(__dirname, '..', '..', 'blurbs', 'action-object.md'), 'utf8')).trim()
  } catch {
    return '(durable object format unavailable — ask the user for it)'
  }
}
