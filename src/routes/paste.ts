...
<label>Store <select name="store">${PASTE_STORES.map(s => `<option>${s}</option>`).join('')}</select></label><br><br>
<label>Title <input name="title" size="60" placeholder="short title"></label><br><br>
<label>Labels (comma-separated, optional) <input name="labels" size="60" placeholder="resume:resumes-zak"></label><br><br>
<textarea name="text" rows="20" cols="90" placeholder="paste the agent block here"></textarea><br><br>
<button type="submit">Create bead</button>
</form></body></html>`
...
pasteRouter.post('/paste', async (req: Request, res: Response) => {
  try {
    const store = String(req.body?.store ?? 'inbox')
    const title = String(req.body?.title ?? 'pasted block').slice(0, 120) || 'pasted block'
    const text = String(req.body?.text ?? '')
    const labels = String(req.body?.labels ?? '').split(',').map((l: string) => l.trim()).filter(Boolean)
    if (!PASTE_STORES.includes(store as typeof PASTE_STORES[number]))
      return void res.status(400).type('text').send(`unknown store (allowlist: ${PASTE_STORES.join(', ')})`)
    if (!text.trim()) return void res.type('html').send(FORM('nothing to save — paste text first'))
    const args = ['create', title, '--description', text, '--label', 'source:paste']
